...
public interface AccountRepository extends JpaRepository<Account, Integer> {
  ...
  // khi select từng trường (k phải select *) cần tạo interfaces để định nghĩa
  // type. VD: TestResponse
  // còn select All thì type là entity luôn
  List<TestResponse> findAllAccounts();

  @Query(value = "SELECT a.* FROM accounts a WHERE a.id = :id",  nativeQuery = true)
  Optional<Account> finBankById(@Param("id") int id);
}
